...
1. 작업한 파일을 가지고
2. 한번 모아서 (``$ git add`` 명령어)
3. 버전으로 남긴다 (``$ git commit`` 명령어)
...
| 작업한 공간            |     | 파일 변경사항을 확인하는 공간 |        | 버전으로 남김          |
...
| 파일 옮기기 _ 명령어 |                   | add |              | commit | 로컬 저장소           | push or pull | 원격 저장소            |
|:------------ | ----------------- | --- | ------------ | ------ | ---------------- | ------------ | ----------------- |
| 파일위치공간       | Working Directory | →   | Staging Area | →      | Local Repository | ↔            | Remote Repository |
| 파일위치확인 명령어   | status            |     | status       |        | log              |              |                   |

우리가 파일을 수정했을 때에도 생겨나게 되는데

또 깃은 파일을 추적을 한 상태인지 (add 를 한 파일인지) 안했다면 Untracked (추적안함)

추적을 한 상태라면 staged (추적함)

그 이후에 커밋을 했다면 Unmodified (안 수정함)

그 이후에 수정을 했다면 Modified (수정함) 
...
| 파일위치공간       | Working Directory    | →   | Staging Area | →      | Local Repository | ↔            | Remote Repository |
| 파일위치확인 명령어   | status               |     | status       |        | log              |              |                   |
| 파일 상태        | untracked            |     | staged       |        | unmodified       |              |                   |
|              | 추적했던 파일 수정__modified |     |              |        |                  |              |                   |
